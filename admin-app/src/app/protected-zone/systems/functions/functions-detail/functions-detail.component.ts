...
export class FunctionsDetailComponent implements OnInit {
    ...
    validation_messages = {
        ...
        id: [{ type: 'required', message: 'Bạn phải nhập mã duy nhất' }],
    ...
    loadDetail(id: any) {
        ...
        this.functionsService.getDetails(id).subscribe(
            (response: any) => {
    ...
    loadParents(id) {
        this.functionsService.getAllByParentId(id).subscribe((response: any) => {
            this.rootFunctions = [];
    ...
    saveChange() {
        ...
        this.blockedPanel = true;
        if (this.entityId) {
            this.functionsService.update(this.entityId, this.entityForm.getRawValue()).subscribe(
                () => {
            ...
            this.functionsService.add(this.entityForm.value).subscribe(
                () => {
