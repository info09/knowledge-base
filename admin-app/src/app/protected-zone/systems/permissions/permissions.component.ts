...
export class PermissionsComponent implements OnInit, OnDestroy {
    ...
    public savePermission() {
        ...
        this.subscription.add(
            this.permissionsService.save(this.selectedRole.id, permissionsUpdateRequest).subscribe(
                () => {
    ...
    loadData(roleId) {
        if (roleId != null) {
            ...
            this.subscription.add(
                this.permissionsService.getFunctionWithCommands().subscribe(
                    (response: any) => {
            ...
            if (checked) {
                ...
                if (parentId === null) {
                    const childFunctions = this.flattenFunctions
                        .filter((x) => x.parentId === functionId)
                        .map((x) => x.id);
